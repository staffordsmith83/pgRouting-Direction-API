...
function route(start, end) {
  const query_sql = `SELECT *, st_transform(geom, ${config_pg.output_srid}) as geom, st_asgeojson(st_transform(geom, ${config_pg.output_srid})) geojson, st_astext(st_transform(geom, ${config_pg.output_srid})) wkt FROM pgr_dijkstra(
...
function distance(start, end) {
  const query_sql = `SELECT * FROM pgr_dijkstra(
...
function closest(lat, lng, buffer, limit) {
  ...
  const query_sql = `SELECT *, st_asgeojson(st_transform(geom, ${config_pg.output_srid})) geojson, st_astext(st_transform(geom, ${config_pg.output_srid})) wkt, ST_Distance(geom, ${point}) distance
...
const topology = async () => {
  ...
  await query(
    `SELECT pgr_createTopology('${config_pg.table}', 0.0001, 'geom', 'id')`,
    () => {
  ...
  await query(
    `SELECT pgr_createVerticesTable('${config_pg.vertices_table}', 'geom', 'source', 'target')`,
    () => {
  ...
  await query(
    `SELECT  pgr_analyzeGraph('${config_pg.table}', 0.0001, 'geom','id', 'source', 'target')`,
    () => {
  ...
  await query(
    `UPDATE ${config_pg.table} SET cost_len = ST_Length(st_transform(geom, 3857))`,
    () => {
